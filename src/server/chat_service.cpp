...
int send_group_message(sqlite3* db, int sender_id, int group_id, const std::string& content) {
    if (!is_group_member(db, group_id, sender_id)) {
        return DB_NOT_GROUP_MEMBER;
    }
...
int get_group_message_history(sqlite3* db, int group_id, std::vector<Message>& messages, int limit, int offset) {
    const char *query = R"(
    SELECT m.message_id, m.sender_id, m.group_id, m.content, m.timestamp, m.is_read, a.username
    ...
    while ((rc =sqlite3_step(stmt)) ==SQLITE_ROW) {
        ...
        message.message_id = sqlite3_column_int(stmt, 0);
        message.sender_id = sqlite3_column_int(stmt, 1);
        message.group_id = sqlite3_column_int(stmt, 2);
        ...
        const unsigned char* username = sqlite3_column_text(stmt, 6);
...
int get_offline_messages(sqlite3* db, int user_id, std::vector<Message>& messages) {
    // Note: Since we removed is_offline, this function might need re-evaluation.
    // However, typically offline messages are just unread messages.
    // But the user specifically asked to remove is_offline field.
    // So we will return all unread direct messages.
    
    const char *query = R"(
    SELECT m.message_id, m.sender_id, m.receiver_id, m.content, m.timestamp, m.is_read, a.username
