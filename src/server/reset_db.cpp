...
void seed_data(sqlite3* db)
{
    ...
    std::string insert_accounts = 
        ...
        "('alice', 'password123', 'active', 'offline', " + std::to_string(week_ago) + "),"
        "('bob', 'bobpass456', 'active', 'online', " + std::to_string(week_ago) + "),"
        "('charlie', 'charlie789', 'active', 'online', " + std::to_string(week_ago) + "),"
        "('diana', 'diana2024', 'active', 'offline', " + std::to_string(day_ago) + "),"
        "('eve', 'eve12345', 'banned', 'offline', " + std::to_string(week_ago) + ");";
    ...
    std::string insert_friendships =
        ...
        "(1, 2, " + std::to_string(week_ago) + "),"  // alice <-> bob
        "(1, 3, " + std::to_string(week_ago) + "),"  // alice <-> charlie
        "(2, 3, " + std::to_string(week_ago) + ");"; // bob <-> charlie
    ...
    std::string insert_friend_requests =
        ...
        "(1, 4, 'pending', " + std::to_string(day_ago) + "),"     // alice -> diana (pending)
        "(2, 5, 'rejected', " + std::to_string(week_ago) + "),"   // bob -> eve (rejected)
        "(4, 1, 'accepted', " + std::to_string(week_ago) + ");";  // diana -> alice (accepted)
    ...
    std::string insert_groups =
        ...
        "('Study Group', 1, " + std::to_string(week_ago) + "),"
        "('Gaming Squad', 2, " + std::to_string(week_ago) + "),"
        "('Work Team', 3, " + std::to_string(day_ago) + ");";
    ...
    std::string insert_group_members =
        ...
        "(1, 1, 'admin', " + std::to_string(week_ago) + "),"    // Study Group: alice (admin)
        "(1, 2, 'member', " + std::to_string(week_ago) + "),"   // Study Group: bob
        "(1, 3, 'member', " + std::to_string(week_ago) + "),"   // Study Group: charlie
        "(2, 2, 'admin', " + std::to_string(week_ago) + "),"    // Gaming Squad: bob (admin)
        "(2, 3, 'member', " + std::to_string(week_ago) + "),"   // Gaming Squad: charlie
        "(3, 3, 'admin', " + std::to_string(day_ago) + "),"     // Work Team: charlie (admin)
        "(3, 1, 'member', " + std::to_string(day_ago) + "),"    // Work Team: alice
        "(3, 4, 'member', " + std::to_string(day_ago) + ");";   // Work Team: diana
    ...
    std::string insert_messages =
        ...
        "(1, 2, NULL, 'Hey Bob, how are you?', " + std::to_string(week_ago) + ", 1, 0),"
        "(2, 1, NULL, 'I''m good Alice, thanks!', " + std::to_string(week_ago) + ", 1, 0),"
        "(3, 1, NULL, 'Hi Alice!', " + std::to_string(day_ago) + ", 0, 1),"  // offline message
        "(1, NULL, 1, 'Welcome to Study Group everyone', " + std::to_string(week_ago) + ", 1, 0),"
        "(2, NULL, 1, 'Thanks for adding me!', " + std::to_string(week_ago) + ", 1, 0),"
        "(4, 1, NULL, 'Can we talk later?', " + std::to_string(day_ago) + ", 0, 1),"  // offline message
        "(2, NULL, 2, 'Ready to play?', " + std::to_string(week_ago) + ", 1, 0),"
        "(3, NULL, 2, 'Let''s go!', " + std::to_string(week_ago) + ", 1, 0);";
    ...
    std::string insert_activity_logs =
        ...
        "(2, 'login', 'User logged in from 127.0.0.1', " + std::to_string(now - 3600) + "),"
        "(1, 'send_message', 'Sent message to user 2', " + std::to_string(now - 3500) + "),"
        "(3, 'join_group', 'Joined group 1', " + std::to_string(week_ago) + "),"
        "(2, 'send_friend_request', 'Sent friend request to user 5', " + std::to_string(week_ago) + "),"
        "(4, 'create_group', 'Created group 3', " + std::to_string(day_ago) + "),"
        "(1, 'accept_friend_request', 'Accepted friend request from user 4', " + std::to_string(week_ago) + "),"
        "(3, 'logout', 'User logged out', " + std::to_string(now - 7200) + ");";
